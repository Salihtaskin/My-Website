...
## v4 Güncellemeleri (Ayarlar paneli, tehdit göstergesi, canlı log, şifre/geçmiş, rozetler, mini quiz)

Bu paket her yeni özelliği admin panelinden açıp kapatabileceğin ve eşik değerlerini değiştirebileceğin bir **Ayarlar** sekmesiyle geliyor — kod değiştirmeden.

### 1) D1 Migration'ı çalıştır

`migration_v4.sql` dosyasının tamamını Cloudflare Dashboard → D1 veritabanın → **Console** sekmesine yapıştırıp çalıştır (migration_v2 ve migration_v3 zaten çalıştırılmış olmalı). 10 tane örnek quiz sorusu otomatik eklenir, admin panelden düzenleyebilir/silebilirsin.

### 2) Yeni admin sekmeleri

- **Ayarlar:** Tehdit Seviyesi Göstergesi, Canlı Aktivite Logu, Kullanıcılar için Quiz, Kullanıcılar için Rozet — her biri ayrı aç/kapa anahtarı. Ayrıca sarı/kırmızı tehdit eşiği, rozet için gereken giriş serisi/sayısı, canlı log yenileme aralığı gibi sayısal değerler buradan değişir. Aynı sekmede quiz sorularını ekleme/aktif-pasif yapma/silme de var.
- **Canlı Log:** giriş denemeleri, IP engellemeleri ve yeni ziyaretleri terminal tarzında akan bir liste halinde gösterir (Ayarlar'daki saniyeye göre kendini yeniler).
- Admin panelinin en üstünde, ayarlardan açıksa, genel bir **tehdit seviyesi rozeti** (yeşil/sarı/kırmızı) görünür.

### 3) Kullanıcı tarafı — "Hesabım" bölümü

Artık sadece admin değil, onaylı her kullanıcı dashboard'da yeni bir **Hesabım** bölümü görüyor:
- Kendi şifresini değiştirme.
- Kendi giriş geçmişi (tarih, IP, cihaz, başarılı/başarısız).
- Rozetler (Ayarlar'dan açıksa): İlk Giriş, X Giriş, X Günlük Seri, Quiz Ustası.
- Mini Siber Güvenlik Quiz (Ayarlar'dan açıksa): soruları cevapla, puan al, skor tablosunda yerini gör.

### 4) Yeni dosyalar özeti

- `migration_v4.sql` — `site_settings`, `quiz_questions`, `quiz_scores` tabloları
- `functions/api/admin/settings.js`, `functions/api/settings.js` — ayarları okuma/yazma
- `functions/api/admin/threat-level.js` — tehdit seviyesi hesaplama
- `functions/api/admin/live-feed.js` — canlı log verisi
- `functions/api/change-password.js`, `functions/api/my-login-history.js`, `functions/api/my-badges.js` — kullanıcı hesabı
- `functions/api/quiz/questions.js`, `submit.js`, `leaderboard.js` — kullanıcı quiz akışı
